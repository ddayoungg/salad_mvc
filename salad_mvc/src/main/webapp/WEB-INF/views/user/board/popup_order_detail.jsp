...
<div class="board_write_popup">
	<div class="ly_tit"><h3>주문/배송상세</h3></div>
    ...
    <div class="scroll_box">
    	<div class="board_write_box">
			...
			<c:forEach varStatus="status" var="order" items="${ orderList }">
				<tr data-order-no="2210251644000099" data-order-goodsno="442263" data-order-status="r3" data-order-userhandlesno="8969">
					<c:if test="${status.first}">
						<td rowspan="${fn:length(orderList)}" class="order_day_num aaa" style="text-align: center;"><br/>
						   <em><c:out value="${order.orderDate}"/></em><br/>
						   <span>주문번호 : <c:out value="${order.orderNum}"/></span><br/>
						   <span><c:out value="${order.statusName}"/></span>
						</td>
					</c:if>
					<td class="td_left">
						<div class="pick_add_cont">
					       	<span class="pick_add_img">
								<img src="http://localhost/salad_mvc/common/images/product/${ order.thum }" width="50" class="middle"  />
					    	</span>
					        <div class="pick_add_info">
								<em><c:out value="${ order.prdName }"/></em>
					        </div>
					        </div>
					        <!-- //pick_add_info -->
					    	</td>
			...
			<div class="board_write_box" style="margin-top: 20px">
			   <table class="board_write_table">
			    <colgroup>
			     <col style="height:15%;">
			     <col style="height:85%;">
		...
		<c:if test="${ existAddrFlag eq '0' }">
           <div class="mypage_zone_tit"><h4>배송지 정보</h4></div>
            ...
            <div class="btn_center_box" style="margin-top: 60px">
            	<c:if test="${orderList.get(0).statusName eq '주문 접수'}">
	            <!-- 취소 버튼 -->
            	<a href="#void" id="cancelBtn" class="btn_ly_write_ok" style="background-color: white; color: #333;"><strong>주문취소</strong></a>
	            </c:if>
	            <!-- 확인 버튼 -->
            	<a href="#void" id="closeBtn" class="btn_ly_write_ok"><strong>확인</strong></a>
            </div>
            </div>
        </div>
        <!-- //order_view_info -->
