...
function setMyTotal() {//나의 전체 찜, 후기, 상품문의 건수
    ...
    <script type="text/javascript" src="http://salad.sist.co.kr/resources/js/jquery/datetimepicker/bootstrap-datetimepicker.min.js?v=2020120404"></script>
    <!-- Add script : end -->
...
<script type="text/javascript" src="http://salad.sist.co.kr/resources/js/naver/naverCommonInflowScript.js?Path=/mypage/order_list.jsp&amp;Referer=https://www.pocketsalad.co.kr:443&amp;AccountID=s_2dc21239d6a&amp;Inflow=pocketsalad.co.kr" id="naver-common-inflow-script"></script>
...
<!-- //mypage_top_info -->
        ...
        <div class="mypage_lately_info">
            <div class="mypage_zone_tit">
                <h3>주문/배송상세<!--  <strong class="order_num_view">(주문번호 : 2210251644000099 )</strong> --></h3>
            </div>
    ...
    <table>
        ...
        <tbody>
		<c:forEach varStatus="status" var="order" items="${ orderList }">
			<tr data-order-no="2210251644000099" data-order-goodsno="442263" data-order-status="r3" data-order-userhandlesno="8969">
				<c:if test="${status.first}">
					<td rowspan="${fn:length(orderList)}" class="order_day_num aaa"><br/>
			            <em><c:out value="${order.orderDate}"/></em><br/>
			            <span>주문번호 : <c:out value="${order.orderNum}"/></span><br/>
			            <span><c:out value="${order.orderStatus eq '4'?'취소요청':'취소확정'}"/></span>
			        </td>
				</c:if>
		    	<td class="td_left">
		        <div class="pick_add_cont">
		            <span class="pick_add_img">
						<a href="http://salad.sist.co.kr/goods/goods_view.do?prdNum=${ order.prdNum }"><img src="http://salad.sist.co.kr/common/images/product/${ order.thum }" width="50" class="middle"  /></a>
		            </span>
		            <div class="pick_add_info">
						<a href="http://salad.sist.co.kr/goods/goods_view.do?prdNum=${ order.prdNum }"><em><c:out value="${ order.prdName }"/></em></a>
		            </div>
		        </div>
		        <!-- //pick_add_info -->
		    	</td>
		    	<td><c:out value="${ order.orderCnt }"/>개</td>
		    	<td><strong><fmt:formatNumber value='${ (order.prdPrice-(order.prdPrice*(order.prdDiscount/100))) }' pattern="#,#00"/></strong>원</td>
			</tr>
		</c:forEach>
        </tbody>
    </table>
...
<div class="mypage_table_type" style="margin-top: 20px">
   ...
   <tbody>
    <tr>
	  ...
	  <th style="text-align: center">총 할인금액</th>
	  <th style="text-align: center">배송비</th>
	  <th style="text-align: center">합계</th>
    </tr>
    <tr>
      <td style="text-align: center"><strong><span><fmt:formatNumber value='${ totalPrice.totalPrdPrice }' pattern="#,#00"/></span></strong>원</td>
      <td style="text-align: center"><strong><span><fmt:formatNumber value='${ totalPrice.totalPrdDiscount }' pattern="#,#00"/></span></strong>원</td>
      <td style="text-align: center"><strong><span><fmt:formatNumber value='3000' pattern="#,#00"/></span></strong>원</td>
      <td style="text-align: center"><strong><span><fmt:formatNumber value='${ totalPrice.orderTotalPrice }' pattern="#,#00"/></span></strong>원</td>
    </tr>
  </tbody>
  </table>
</div>

<c:if test="${ totalPrice.orderStatus eq '5' }"><!-- 취소확정일 경우 환불 금액 표시 -->
<div class="mypage_table_type" style="margin-top: 20px">
   <table class="table_left">
    <colgroup>
     <col style="height:15%;">
   ...
   <tbody>
    <tr>
	  <th style="text-align: center">환불 금액</th>
    </tr>
    <tr>
      <td style="text-align: center"><strong><span><fmt:formatNumber value='${ totalPrice.orderTotalPrice }' pattern="#,#00"/></span></strong>원</td>
    </tr>
...
<script type="text/javascript" src="/data/skin/front/kaimen_pc_n/js/gd_board_goods.js" charset="utf-8"></script>
                <!--// 주문상품 리스트 -->
            </div>
        ...
        <div class="order_view_info">
                ...
                <div class="mypage_table_type">
                    <table class="table_left">
                        ...
                        <tr>
                            ...
                            <td><c:out value="${fn:substring(myData.phone,0,3)}-${fn:substring(myData.phone,3,7)}-${fn:substring(myData.phone,7,11)}"/></td>
                        </tr>
                        <tr>
                            <th scope="row">이메일</th>
                            <td><c:out value="${ myData.email }"/></td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="mypage_table_type">
                    <table class="table_left">
                        ...
                        <tr>
                            <th>주소</th>
                            <td>
                            	(<c:out value="${ myData.zipcode }"/>)<br/>
                            	<c:out value="${ myData.addr }"/>&nbsp;<c:out value="${ myData.deAddr }"/>
                            </td>
                        </tr>
						<tr>
							<th>받으실 분</th>
							<td>
								<c:out value="${ myData.name }"/><br/>
								<c:out value="${fn:substring(myData.phone,0,3)}-${fn:substring(myData.phone,3,7)}-${fn:substring(myData.phone,7,11)}"/>
							</td>
						</tr>
                        <tr>
                            <th scope="row">배송 요청 사항</th>
                            <td><c:out value="${ myData.deliReq }"/></td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            </c:if>
			<c:if test="${ existAddrFlag eq '1' }">
                <div class="mypage_zone_tit">
                    <h4>배송지 정보</h4>
                </div>
                ...
                <div class="mypage_table_type">
                    <table class="table_left">
						...
						<tr>
							<th>받으실 분</th>
							<td>
								<c:out value="${ deliData.receiver }"/><br/>
								<c:out value="${fn:substring(deliData.deliPhone,0,3)}-${fn:substring(deliData.deliPhone,3,7)}-${fn:substring(deliData.deliPhone,7,11)}"/>
							</td>
						</tr>
                        <tr>
                            <th scope="row">배송 요청 사항</th>
                            <td><c:out value="${ deliData.deliReq }"/></td>
                        </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <!-- 확인 버튼 -->
            <div class="btn_center_box" style="margin-top: 60px">
            	<a href="#void" id="backBtn" class="btn_ly_write_ok"><strong>확인</strong></a>
            </div>
            </div>
        </div>
        <!-- //order_view_info -->
